# ticker_app_v9.py
# v9: persistent save to Railway volume (Option B), reactive sliders (no manual refresh),
# improved error handling, preseason protection, user-facing messages, loading indicators,
# protection against missing opponents.
...
st.set_page_config(layout="wide", page_title="FPL Season Ticker v9")

# ---------------------
# Configuration
# ---------------------
VOLUME_PATH = os.environ.get("RAILWAY_VOLUME_PATH") or os.environ.get("PERSISTENT_VOLUME_PATH") or ""
SAVE_FILENAME = "saved_difficulties.csv"
if VOLUME_PATH:
    SAVE_FILEPATH = os.path.join(VOLUME_PATH, SAVE_FILENAME)
else:
    SAVE_FILEPATH = None

# Enforce Option B: require persistent volume path to be set (clear error if not)
if SAVE_FILEPATH is None:
    st.error(
        "Persistent storage required (Option B). "
        "Please set the environment variable RAILWAY_VOLUME_PATH to a directory (e.g. /data). "
        "Without it this app will not persist difficulties across restarts."
    )
    st.stop()
...
def load_fpl_data() -> Tuple[pd.DataFrame, List[str], Dict[int, Dict[str,str]]]:
    """
    Returns (fixtures_df, sorted_team_codes_list, teams_full_map)
    On failure returns (empty_df, [], {}).
    """
    try:
    ...
    # Build teams map from bootstrap when available
    if isinstance(boot, dict) and boot.get("teams"):
        try:
            ...
            teams = {}
    # If bootstrap missing, infer team ids from fixtures and create fallback short names
    ...
    for f in fixtures:
        try:
            ...
            team_h = f.get("team_h")
            team_a = f.get("team_a")
            # Ensure fallback if tid not present
...
# ---------------------
# Load FPL data with spinner and friendly errors
# ---------------------
...
if df.empty or len(team_codes) == 0 or not teams_full:
    ...
    st.info(
        "If you deployed to Railway, ensure outbound HTTP to fantasy.premierleague.com is allowed. "
        "Try again later."
    )
    st.stop()

# ---------------------
# Defaults and persistence helpers (Railway volume)
# ---------------------
DEFAULT_VALUES = {t: {"Home": 1250, "Away": 1350} for t in team_codes}

def load_saved_difficulties_from_disk() -> pd.DataFrame:
    """Load saved difficulties CSV from persistent volume. Returns DataFrame or None on missing file."""
    try:
        if os.path.exists(SAVE_FILEPATH):
            ...
            if "Home" in df_saved.columns and "Away" in df_saved.columns:
                ...
                st.error("Saved difficulties file is malformed (missing 'Home' or 'Away' columns).")
                return None
        else:
            return None
    except Exception as e:
        st.error(f"Failed to read saved difficulties from persistent storage: {e}")
        return None

def save_difficulties_to_disk(df_to_save: pd.DataFrame):
    """Write difficulties DataFrame to persistent CSV. Shows user-facing messages on success/failure."""
    try:
...
# Provide a simple helper to ensure difficulties index covers all team_codes
def ensure_difficulties_cover_teams():
    ...
    # reorder index to team_codes order
...
with st.sidebar:
    ...
    with st.expander("Difficulty Sliders (adjust and Apply)"):
        ...
        # Initialize slider session keys from current difficulties (if not present)
        for t in team_codes:
            kh = f"slider_home_{t}"
            ka = f"slider_away_{t}"
        ...
        # render sliders (they update session_state live)
        for t in team_codes:
        ...
        # Apply button: read slider values, update session_state difficulties and save to disk
        if st.button("Apply sliders (save & apply)"):
            with st.spinner("Applying sliders and saving..."):
                try:
                    ...
                    save_difficulties_to_disk(new_df)
                    st.success("Sliders applied and saved (changes visible immediately).")
                except Exception as e:
                    st.error(f"Failed to apply/save sliders: {e}")

    st.markdown("---")
    # Manual import/export
    if st.button("Export difficulties (download CSV)"):
    ...
    if uploaded is not None:
        try:
            ...
            save_difficulties_to_disk(imported)
