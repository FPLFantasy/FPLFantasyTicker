...
# - Atomic write when saving CSV (write temp file then os.replace)
# - Reactive sliders/table: apply/save now updates UI immediately via st.experimental_rerun()
# - Friendly user messages and spinners around IO
# - Defensive checks & graceful fallbacks retained from v9
...
# ---------------------
# Configuration: pick persistent volume path if available
# Priority:
# 1) If /data exists (you mounted a Railway volume there) -> use it
# 2) Else if env var RAILWAY_VOLUME_PATH or PERSISTENT_VOLUME_PATH set -> use it
# 3) Else: show clear error and stop (Option B requires a volume)
# ---------------------
PREFERRED_MOUNT = "/data"
env_path = os.environ.get("RAILWAY_VOLUME_PATH") or os.environ.get("PERSISTENT_VOLUME_PATH") or ""
if os.path.exists(PREFERRED_MOUNT) and os.path.isdir(PREFERRED_MOUNT):
    VOLUME_PATH = PREFERRED_MOUNT
elif env_path:
    VOLUME_PATH = env_path
else:
    VOLUME_PATH = ""

SAVE_FILENAME = "saved_difficulties.csv"
SAVE_FILEPATH = os.path.join(VOLUME_PATH, SAVE_FILENAME) if VOLUME_PATH else None

if SAVE_FILEPATH is None:
    st.error(
        "Persistent storage (Option B) is required for this deployment. "
        "Mount a volume at /data or set RAILWAY_VOLUME_PATH environment variable to continue."
    )
    st.stop()
...
def load_saved_difficulties_from_disk() -> pd.DataFrame:
    """Load saved difficulties CSV from persistent volume. Returns DataFrame or None on missing/malformed file."""
    try:
        if os.path.exists(SAVE_FILEPATH):
            ...
            if "Home" in df_saved.columns and "Away" in df_saved.columns:
                ...
                st.error("Saved difficulties file is malformed (missing 'Home' or 'Away' columns). Using defaults.")
                return None
        else:
            return None
    except Exception as e:
        st.error(f"Failed to read saved difficulties from persistent storage: {e}. Using defaults.")
        return None

def atomic_save_difficulties(df_to_save: pd.DataFrame):
    """Atomically write the CSV to disk (write temp file then replace). Shows UI messages on success/failure."""
    try:
        os.makedirs(os.path.dirname(SAVE_FILEPATH), exist_ok=True)
        # ensure index name is "Team"
        df_to_save = df_to_save.copy()
        df_to_save.index.name = "Team"
        # write to temporary file in same directory then move
        dirpath = os.path.dirname(SAVE_FILEPATH)
        with tempfile.NamedTemporaryFile(mode="w", dir=dirpath, delete=False) as tf:
            tmpname = tf.name
            df_to_save.to_csv(tmpname, index=True)
        os.replace(tmpname, SAVE_FILEPATH)  # atomic on most OSes
